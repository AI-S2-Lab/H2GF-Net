...
class MULT:
    ...
    def __init__(self, args, data, model):
        ...
        for batch in self.train_dataloader:
            label_list += torch.flatten(batch['label_ids']).tolist()
    ...
    def _train(self, args): 
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(self.train_dataloader, desc="Iteration")):
                ...
                text_semantic_feats = batch['text_semantic_feats'].to(self.device)
                audio_semantic_feats = batch['audio_semantic_feats'].to(self.device)
                video_semantic_feats = batch['video_semantic_feats'].to(self.device)
                label_ids = batch['label_ids'].to(self.device)

                with torch.set_grad_enabled(True):

                    preds, last_hiddens = self.model(text_feats, video_feats, audio_feats, text_semantic_feats, audio_semantic_feats, video_semantic_feats)

                    print(">>> preds.shape:", preds.shape, "label_ids.shape:", label_ids.shape)
    ...
    def _get_outputs(self, args, mode = 'eval', return_sample_results = False, show_results = False):
        ...
        for batch in tqdm(dataloader, desc="Iteration"):
            ...
            with torch.set_grad_enabled(False):
                ...
                logits, last_hiddens = self.model(text_feats, video_feats, audio_feats, text_semantic_feats, audio_semantic_feats, video_semantic_feats)
