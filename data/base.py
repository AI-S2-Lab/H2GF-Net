...
from .text_semantic_pre import TextSemanticDataset
from .audio_semantic_pre import AudioSemanticDataset
from .video_semantic_pre import VideoSemanticDataset
from .mm_pre import MMDataset
...
class DataManager:
    ...
    def __init__(self, args, logger_name = 'Multimodal Intent Recognition'):
        ...
        args.text_feat_dim, args.video_feat_dim, args.audio_feat_dim, args.text_semantic_feat_dim, args.audio_semantic_feat_dim, args.video_semantic_feat_dim = \
            self.benchmarks['feat_dims']['text'], self.benchmarks['feat_dims']['video'], self.benchmarks['feat_dims']['audio'], self.benchmarks['feat_dims']['text_semantic'], self.benchmarks['feat_dims']['audio_semantic'], self.benchmarks['feat_dims']['video_semantic']
        args.text_seq_len, args.video_seq_len, args.audio_seq_len, args.text_semantic_seq_len, args.audio_semantic_seq_len, args.video_semantic_seq_len = \
            self.benchmarks['max_seq_lengths']['text'], self.benchmarks['max_seq_lengths']['video'], self.benchmarks['max_seq_lengths']['audio'], self.benchmarks['max_seq_lengths']['text_semantic'], self.benchmarks['max_seq_lengths']['audio_semantic'], self.benchmarks['max_seq_lengths']['video_semantic']
    ...
    def _get_unimodal_feats(self, args, attrs):
        ...
        text_semantic_feats = TextSemanticDataset(args, attrs).feats
        audio_semantic_feats = AudioSemanticDataset(args, attrs).feats
        video_semantic_feats = VideoSemanticDataset(args, attrs).feats

        return {
            'text': text_feats,
            'video': video_feats,
            'audio': audio_feats,
            'text_semantic': text_semantic_feats,
            'audio_semantic': audio_semantic_feats,
            'video_semantic': video_semantic_feats
        }
    ...
    def _get_multimodal_data(self, args):
        ...
        text_semantic_data = self.unimodal_feats['text_semantic']
        audio_semantic_data = self.unimodal_feats['audio_semantic']
        video_semantic_data = self.unimodal_feats['video_semantic']
        
        mm_train_data = MMDataset(self.train_label_ids, text_data['train'], video_data['train'], audio_data['train'], text_semantic_data['train'], audio_semantic_data['train'], video_semantic_data['train'])
        mm_dev_data = MMDataset(self.dev_label_ids, text_data['dev'], video_data['dev'], audio_data['dev'], text_semantic_data['dev'], audio_semantic_data['dev'], video_semantic_data['dev'])
        mm_test_data = MMDataset(self.test_label_ids, text_data['test'], video_data['test'], audio_data['test'], text_semantic_data['test'], audio_semantic_data['test'], video_semantic_data['test'])
