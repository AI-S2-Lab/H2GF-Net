...
__all__ = ['TextSemanticDataset']

class TextSemanticDataset:
    ...
    def _get_feats(self, args, base_attrs):
        ...
        self.logger.info('Generate Text Semantic Features Begin...')
        ...
        self.logger.info('Generate Text Semantic Features Finished...')
    ...
    def _get_bert_feats(self, args, examples, base_attrs):
        ...
        max_seq_length = base_attrs["benchmarks"]['max_seq_lengths']['text_semantic']
    ...
    def get_examples(self, data_dir, mode):
        if mode == 'train':
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, "text_desc_train.tsv")), "train")
        elif mode == 'dev':
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, "text_desc_dev.tsv")), "train")
        elif mode == 'test':
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, "text_desc_test.tsv")), "test")

    def _create_examples(self, lines, set_type):
        """Creates examples for the training and dev sets."""
        examples = []
        for (i, line) in enumerate(lines):      # 通过条件判断来跳过第一行数据。通常情况下，第一行可能是标题行或列名，不包含实际的数据。
            if i == 0:
                continue

            guid = "%s-%s" % (set_type, i)
            text_a = line[1]        #读取第几列的内容
